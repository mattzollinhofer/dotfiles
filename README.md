...
- `duti` - sets default apps for file types (makes `.md` open in nvim)
...
### 4. Markdown Opener (macOS)
install.sh builds an "Open in nvim" wrapper app at `~/Applications/Open in nvim.app` and sets it as the default handler for `.md` files (via `duti`), so double-clicking a markdown file in Finder opens it in nvim in a fresh iTerm window. The launch logic lives in `bin/open-in-nvim`; the app is generated by `macos/build-finder-opener` (it is not committed — it's rebuilt on each install).

The first time you double-click a markdown file, macOS asks *"Open in nvim wants to control iTerm.app"* — click **Allow**. This is a per-machine permission (stored in macOS TCC) and cannot be captured in the dotfiles.
